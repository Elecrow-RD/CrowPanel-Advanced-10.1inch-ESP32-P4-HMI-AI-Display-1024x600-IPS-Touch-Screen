...
/* Match the smooth Brookesia camera example: keep four capture buffers so
 * CSI can continue filling while LVGL synchronously displays one frame. */
#define CAMERA_BUFFER_COUNT 4
uint8_t *cam_buffer[CAMERA_BUFFER_COUNT];
size_t cam_buffer_size[CAMERA_BUFFER_COUNT];
 ...
 *
 * Called by the stream task for every captured frame. The capture buffer is
 * attached directly to the LVGL canvas and refreshed synchronously before
 * the V4L2 driver is allowed to reuse that buffer.
 */
                                         ...
                                         size_t camera_buf_len)
{
    ...
    /* Match the Brookesia example: display the ISP RGB565 buffer directly,
     * then finish LVGL refresh before V4L2 can reuse that capture buffer. */
    if (src_x != 0 || src_y != 0 || out_w != camera_buf_hes || out_h != camera_buf_ves) {
        CAMERA_ERROR("zero-copy preview requires a full-size camera frame");
        return;
    }
    if (lvgl_port_lock(100)) {
        lv_canvas_set_buffer(camera_obj, camera_buf, out_w, out_h,
...
int camera_work(void)
{
    ...
    err = camera_video_set_bufs(camera_video_id, CAMERA_BUFFER_COUNT);
    if (err != ESP_OK) {
